...
- `agent-browser`, `skill-creator`, `antigravity` (the `agy` CLI wrapper, replaces the old gemini-cli skill), `codex` (the same playbooks for the OpenAI Codex CLI, written fresh to replace gstack's), `cli-for-agents` (from cursor/plugins), the marketing pack, and the ui.sh design skills: as installed from their upstreams.
