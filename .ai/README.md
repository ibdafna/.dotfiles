...
- `rigorous-mode`, `how`, `why`, `architect`, `arena`, `swarm`, `interrogate`, the `principle-*` set, and the rest of pstack: a harness-agnostic fork of the `pstack` plugin from cursor/plugins (entry point renamed from `poteto-mode`). Its two agents live in `claude/agents/`.
...
- `agent-browser`, `skill-creator`, the marketing pack, and the ui.sh design skills: as installed from their upstreams.
