...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `verify-*/` under a project skills directory from the Paths table in `../rigorous-mode/references/harness.md`; check every harness's directory, they may be symlinks to one). Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target.
