...
This skill enables Claude Code to effectively orchestrate Gemini CLI (v0.16.0+) with Gemini 3.1 Pro for code generation, review, analysis, and specialized tasks.

## When to Use This Skill

By default, your chosen model should be Gemini 3.1 Pro Preview, with a fallback to Gemini 3 Flash.

Use the exact CLI model id `gemini-3.1-pro-preview` for Gemini 3.1 Pro Preview in this environment. Do not use aliases such as `gemini-3.1-pro`, `gemini-pro-3.1`, or `gemini-3-pro`; they return `ModelNotFoundError`.
...
6. **Frontend Design and Implementation**
   - Gemini 3.1 Pro has excellent frontend design capaabilities
   - Excellent for animations, sophisticated SVGs, blurs etc
   - Great for HTML/React/Tailwind CSS and frontend-oriented JavaScript
   - Fantastic animated SVG capabilities!
...
```bash
gemini -m gemini-3.1-pro-preview -p "[prompt]" --yolo -o text 2>&1
```

Key flags:
- `-m gemini-3.1-pro-preview`: Required model id for Gemini 3.1 Pro Preview
- `-p` or `--prompt`: Non-interactive/headless mode
...
**Rate Limits**: Free tier has 60 requests/min, 1000/day. CLI auto-retries with backoff. Expect messages like "quota will reset after Xs". We have a paid subscription -- feel free to use abundantly!
