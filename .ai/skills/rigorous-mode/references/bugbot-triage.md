...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles review-bot comments. "Bugbot" below stands for any automated reviewer: Cursor's Bugbot, Claude code review, Codex review, or a security-review bot. The goal is not to ignore the bot by default. The goal is to stop treating every comment as a required code change.
