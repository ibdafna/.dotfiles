---
name: rigorous-mode
description: Rigorous engineering mode for concise, detailed responses, deliberate subagents, unslopped prose, simple code, and verified work. Routes any non-trivial task to a playbook and the principles. Use for /rigorous-mode, "rigorous mode", or requests to work with rigor.
disable-model-invocation: true
mode: true
icon: crown
color: yellow
reminder: New task? Playbook match or rigor needed -> apply /rigorous-mode. Casual turn or user opts out -> don't.
---

# Rigorous mode

This skill runs on Cursor, Claude Code, and Codex CLI. Tool names below are capabilities; `references/harness.md` maps each to the tool in your harness, and `references/models.md` resolves every model role. Read both once per session.
...
- About to ask the user (the structured-question tool in `references/harness.md`) on a "which approach", "how should I", or "what should this do" fork → classify it before you ask. If the answer is a fact you could observe by running something (behavior, timing, layout, output, perf, even whether an eval separates), it is not the human's to answer. Sketch it via the Prototype playbook (`playbooks/prototype.md`) and let the result decide. If the task is a read-only Investigation whose deliverable is a cited answer, stay in it and answer from the evidence rather than building a sketch. Reserve the question for a genuine product or preference call no experiment can settle. The ask is the slow path. A throwaway probe usually answers faster, and it hands the human a result to react to instead of a decision to make.
- Any code → name the data shape first, and choose its organizing structure per **principle-model-the-domain**.
- Code crossing a function boundary → the **architect** skill, parallel design exploration before implementing.
- Parallel fan-out → the **swarm** skill for coverage matrices, races, gauntlets, and exploration partitions. Use **arena** for design or code bakeoffs with base selection and grafting.
- Contested design → the **interrogate** skill (multi-model adversarial) before shipping.
- Nontrivial multi-step → write the throughput checkpoint (Feature step 3).
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows your harness's skill-authoring skill (`references/harness.md`).
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit and before review → the **no-comments** skill (`/no-comments`). Its slop pass runs before each commit; its comment audit runs before review.
- Shipping UI / IDE / CLI → the project's verification skill (`verify-<app>`). When the project has none, generate it with **create-verification-skill** first. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
- Any PR-status request → the **Babysit** playbook (`playbooks/babysit.md`), and not a harness-provided babysit skill whose description matches the same words. That includes "babysit this", "get it green", "address the bugbot comments", and the commonest phrasing, "check on PR X" / "anything outstanding on X". Never triggered by merely opening a PR. Declare its mode before polling; the playbook's step 1 owns the request-to-mode mapping. Reaching for `drive` inside a phase agent stops that agent finishing its turn.
- Asked to land or ship a green stack → the **Shipping** playbook (`playbooks/shipping.md`). Green is not safe. Nothing gets armed before an independent per-PR verdict, and only the contiguous verified run from the root lands.
- A review bot (Bugbot, Claude code review, Codex review) or the agentic security review commented → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/bugbot-triage.md`.
...
**Use the `rigorous-agent` agent for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). `references/harness.md` gives the identifier form for your harness. `/rigorous-mode` and `rigorous-agent` route through the same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) prescribe a generic worker for diverse review; respect what the skill prescribes, don't override to `rigorous-agent`.

**Defaults for every spawn.** Background, not blocking. A mode that keeps MCP tools when the worker needs them. File pointers, not inlined context. An explicit model per role, resolved per `references/models.md`, which also tiers code delegates by difficulty. Per-role lines override the defaults in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`).
