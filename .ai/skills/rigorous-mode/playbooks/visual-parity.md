...
4. Verify each component against its baseline via image diff on the matching surface via the project's `verify-<app>` skill. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. the harness's wake mechanism per component until the diff is zero.
