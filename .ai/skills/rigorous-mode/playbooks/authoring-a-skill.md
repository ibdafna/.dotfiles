...
1. Use your harness's skill-authoring skill (`../references/harness.md`). Without one, author by hand against the rules below.
2. Validate the skill: frontmatter has `name` (kebab-case, equal to the directory name) and `description`, no harness-specific tool names in the body (capabilities plus a pointer to `harness.md` instead), referenced files exist, cross-skill links resolve.
