...
1. Capture a baseline trace via the project's `verify-<app>` skill.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent on the `perf-issue` model role (see `../references/models.md`); review the diff. Capture a post-fix trace.
