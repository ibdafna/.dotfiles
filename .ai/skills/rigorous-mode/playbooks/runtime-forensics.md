...
1. Capture the live signal on the matching surface via the project's `verify-<app>` skill: a CPU profile for a spinning process, a heap snapshot for a leak, a CDP trace for a visual glitch. A real artifact, not a guess.
