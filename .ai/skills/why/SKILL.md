...
Before spawning investigators, list the MCP servers available in this session: the tool list, a tool-search or deferred-tool index when the harness has one, and the harness's MCP config (Cursor's `mcps/` directory or `mcp.json`, Claude Code's `claude mcp list`, Codex's `codex mcp list`).
...
Subagent config (each): a generic worker on the `why investigators` model role, spawned in a mode that keeps MCP tools available (spawn parameters per `../rigorous-mode/references/harness.md`). **Do not use a read-only mode that strips MCP access** (`harness.md` says which do); it disables MCP-backed investigators entirely. The source control investigator would be safe read-only, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Spawn one synthesizer subagent: a generic worker on the `why synthesizer` model role, in a mode that keeps MCP tools. The synthesizer's quality check spot-verifies citations, which can require MCP access; a read-only mode that strips MCPs defeats that.
