...
1. Spawn the `comment-sicko` agent (identifier form per `../rigorous-mode/references/harness.md`; when your harness cannot address a named agent, paste the installed `comment-sicko.md` (location per the Paths table in `harness.md`) into the worker's prompt). Pass the scope. Do not restate its rules.
